...
def main():
    ...
    file_path = trials_path + 'pr_error_left_' + user_id + '_results.xlsx'
    ...
    for trial in range(1, int(len(trials_path)/2)-2):
        ...
        # Load finger target
        print('Trial: ', trial)
        finger_target = data_fing[str(trial)]['left_finger_goal']
        ...
        if L and R: # if both hands have been detected
            left_index = [int(L[0][finger_target][0] * width), int(L[0][finger_target][1] * height)]
            right_index = [int(R[0][9][0] * width), int(R[0][9][1] * height)]
            ...
            print('Left hand size in cm: ', hand_size_cm)

            # Call hand size distance function to get the dimension of the hand in pixels
            hand_size_px = int(calculate_hand_size(L[0][1:], width, height))
            print('Left hand size in pixels: ', hand_size_px)
            ...
            new_row = {'user_id': user_id, 'assessment':'left_hand',
