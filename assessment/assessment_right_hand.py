"""
-------------------------------------------------------------------------------
 Title        : assessment_right_hand.py
 Description  : This can be used as an exercise to assess hand proprioception
 Instructions : 1. Place the right hand on your chest
                2. Try to touch each finger with the index fingertip from the left hand
...
class App:
    ...
    # Define a function to hide the left hand
    ...
    # This function saves the experiment's info to a specified directory
    def save_data(self):
        ...
        save_dict = {"trial": trial, "distances": self.all_distances, "times": self.all_times,
                     "all_coord": self.all_coord, "right_finger_goal": right_finger_indeces[finger_number]}
        if endTrial:
            ...
            while cap.isOpened():
                ...
                if results.multi_hand_landmarks:  # this is a list with [landmarks_hand1, landmarks_hand2]
                    ...
                    if len(self.LR) == 2 and self.LR[0][0][0] != self.LR[1][0][0]:
                        ...
                        # Get the index fingertip coordinates from the right hand
                        self.l_idx_8 = self.LR[1][8 + 1]

                        # Get all fingertip coordinates from the left hand
                        self.r_idx_list = [self.LR[0][4 + 1], self.LR[0][8 + 1], self.LR[0][12 + 1],
                                           self.LR[0][16 + 1], self.LR[0][20 + 1]]

                        # Get the target fingertip coordinates from the left hand
                        self.r_idx = self.r_idx_list[finger_number]
                        ...
                        if 60 * wfactor - 30 * wfactor < self.l_idx_8[0] < 60 * wfactor + 30 * wfactor \
                                and height / 2 - 30 * wfactor < self.l_idx_8[1] < height / 2 + 30 * wfactor:
                            if show_green:
                        ...
                        self.dist_xy = int((((self.l_idx_8[0] - self.r_idx[0]) ** 2)
                                           + (self.l_idx_8[1] - self.r_idx[1]) ** 2) ** (1 / 2))
                ...
                if endTrial or sevenSecondStop:
                    ...
                    # Save the frame
                    cv2.imwrite('../data/' + experiment['name'] + '/' + str(trial) + '.JPG',
                                cv2.flip(self.original_image, 1))
                ...
                if trial >= 30:
                    ...
                    # save data when experiment finishes
                    with open("../proprioception_assessment/" + experiment['name'] + "/sample.json", "w") as outfile:
                        json.dump(self.total_dict, outfile, indent=4)
